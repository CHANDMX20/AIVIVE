...
- **Files**:
  - [`AOP.py`](./applications/AOP.py) - Python script for calculating the **percentage error** in gene expression values between real and **GenLocGAN-generated** profiles, followed by **visualization** using a heatmap to identify patterns in gene expression between the two profiles.
...
This contains code for modeling **necrosis** gene expression predictions. 
  ...
  - **Necrosis Model Evaluation** - The model was evaluated on two distinct test sets: one consisting of real *in vivo* profiles and another consisting of optimized synthetic *in vivo* profiles generated by GenLocGAN. 
...
This contains the scripts for performing GenLocGAN model evaluation on the test set using three metrices: cosine similarity, root mean squared error (RMSE), and mean absolute percentage error (MAPE). 
  ...
  - **GenLocGAN** - Measurements were calculated between the optimized synthetic *in vivo* profiles generated by the model and its corresponding real profiles.
...
- **Files**:
  ...
  - [`cosine.py`](./applications/cosine.py) - For calculating the cosine similarity for GenLocGAN group (generated profiles).
  - [`rmse.py`](./applications/rmse.py) - For calculating the rmse for GenLocGAN group (generated profiles).
  - [`mape.py`](./applications/mape.py) - For calculating the mape for GenLocGAN group (generated profiles).
