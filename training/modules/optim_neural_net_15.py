...
train_input = pd.read_csv('path/to/generator1_encoded_prediction_9962160_VivoGenerator.csv', low_memory = False).iloc[:, 1:]  #path to generated in vivo training samples
train_output = pd.read_csv('path/to/vivo_train.csv', low_memory = False).iloc[:, 1:]  #path to real in vivo training data
test_input = pd.read_csv('/path/to/generator1_encoded_prediction_9962160_vivoGenerator_test.csv', low_memory=False).iloc[:, 1:]  # path to generated in vivo test samples
...
cols = pd.read_csv('path/to/module15_genes.csv').PROBEID.unique()
print(len(cols))                   
...
def save_final_model(model, predsPath, step):
    ...
    filename = predsPath + f'/model15_final_epoch_{step:06d}.h5'
