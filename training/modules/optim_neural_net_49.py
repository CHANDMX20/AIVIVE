...
cols = pd.read_csv('path/to/module49_genes.csv').PROBEID.unique()
print(len(cols))                   
...
# Apply Attention Layer
 ...
 # Second Dense Layer with 512 nodes
    x = Dense(512, activation='relu', kernel_regularizer=L2(0.1))(attention_output)
...
def save_final_model(model, predsPath, step):
    ...
    filename = predsPath + f'/model49_final_epoch_{step:06d}.h5'
...
predsPath = path+'/predictions_decoded/train/module49'

#generated csv: vivo train preds for module 49
number = '9962160'
vivo_opt_gen_train = save_preds(predictions, X_train, train_input, predsPath+ '/opt_gen_train' + number + '_Vivo_49.csv')
